...
### Что нужно доделать:
- [x] **Авто-переключение ноды при падении скорости** (реализовано 2026-03-24):
  - `_check_auto_switch()` в `_on_live_metrics` — проверяет `down_bps` без I/O
  - InfoBar уведомление 30 сек: "Скорость упала. Переключение на {node.name}..."
  - Настройки: `auto_switch_enabled`, `auto_switch_threshold_kbps` (50), `auto_switch_delay_sec` (30), `auto_switch_cooldown_sec` (60)
  - Умный выбор ноды: сначала alive с лучшей скоростью, потом по пингу, потом round-robin
  - Не переключается если 1 нода, если idle (нет активного трафика), или в процессе switching/reconnecting
