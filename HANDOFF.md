...
### Что нужно доделать:
- [ ] **Авто-переключение ноды при падении скорости:**
  - Мониторить `down_bps` из live_metrics. Если ниже порога (напр. 50 KB/s) дольше N секунд (напр. 30с) → hot-swap на следующую ноду
  - Показать InfoBar уведомление: "Скорость упала. Переключение на {node.name}..." — держать 30 секунд
  - **КРИТИЧНО: мониторинг НЕ должен тормозить скорость!** Только читать уже собранные метрики, не делать никаких HTTP/API запросов. Просто проверять `self._last_down_bps` в контроллере
  - Порог и задержка настраиваемые в AppSettings
  - Cooldown между переключениями (напр. 60с) чтобы не зациклиться
  - Не переключаться если всего 1 нода
